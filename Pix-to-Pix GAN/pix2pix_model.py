...
def summarize_performance(step, g_model,  d_model,gan_model, dataset, n_samples=3):
    ...
    filename_gan = 'model_gan_%06d.h5' % (step+1)
    filename_g   = 'model_g_%06d.h5' % (step+1)
    filename_d   = 'model_d_%06d.h5' % (step+1)
    #g_model.save(filename2)
    #def save(gan_model, g_model, d_model):
    d_model.trainable = False
    gan_model.save(filename_gan, 'gan_model')
    d_model.trainable = True
    g_model.save(filename_g, 'g_model')
    d_model.save(filename_d, 'd_model')
    
    #print('>Saved: %s and %s' % (filename1, filename_gan,filename_g,filename_d))
